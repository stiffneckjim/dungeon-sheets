...
def make_gm_sheet(
    ...
    if output_format == "pdf":
        ...
        try:
            ...
            log.warning("``lualatex`` not available. Skipping %s", basename)
...
def latex_character_sheet(character, basename, debug=False, paper_size="letter"):
    """Render a character sheet using the latex character sheet template."""
    ...
    tex = jinja_env.get_template("latex_character_sheet.tex").render(
...
def make_character_sheet(
    ...
    if output_format == "pdf":
        ...
        try:
            ...
            log.warning("``lualatex`` not available. Skipping features for %s", character.name)
    elif output_format == "epub":
        epub.create_epub(
