    ...
    for yaml_file in _resolve_yaml_sources(yaml_path):
        for entry in _load_yaml_list_entries(yaml_file, "monster"):
            ...
            attrs = {
                ...
                "name": entry.get("name", class_name),
                "description": entry.get("creature_type", ""),
...
def load_yaml_weapon_classes(yaml_path, base_class, bases_map=None, module=None):
    ...
    base_class : type
        The fallback base weapon class to inherit from when *weapon_type* is not
        found in *bases_map*.
    bases_map : dict[str, tuple[type, ...]], optional
        Mapping from the ``weapon_type`` string used in YAML entries to the tuple
        of base classes to use for that type.  When *None* every class inherits
        directly from *base_class*.
    module : str, optional
        Value to assign to ``__module__`` on each generated class.  Pass
        ``__name__`` from the calling module so that introspection and repr
        reflect where the class is actually injected, not this helper module.
    ...
    for yaml_file in _resolve_yaml_sources(yaml_path):
        for entry in _load_yaml_list_entries(yaml_file, "weapon"):
            ...
            if bases_map and weapon_type in bases_map:
                bases = bases_map[weapon_type]
            else:
                bases = (base_class,)

            attrs = {
                "__doc__": entry.get(
                    "description", f"{entry.get('name', class_name)} weapon loaded from YAML."
                ),
                "name": entry.get("name", class_name),
